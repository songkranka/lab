package th.co.pt.ptgapp.dao.task;

public class LtrDtlDto {

    private String ltrNo;
    private String ltrNoDtl;

    private String idAnalyze;
    private String idMaterial;
    private String role;
    private String color;
    private String colorDesc;
    private String result;
    private String flagSave;
    private String visual;
    private String visualDesc;
    private String createBy;
    private String updateBy;
    private String flagResult;

    public String getLtrNo() {
        return ltrNo;
    }

    public void setLtrNo(String ltrNo) {
        this.ltrNo = ltrNo;
    }

    public String getLtrNoDtl() {
        return ltrNoDtl;
    }

    public void setLtrNoDtl(String ltrNoDtl) {
        this.ltrNoDtl = ltrNoDtl;
    }

    public String getIdMaterial() {
        return idMaterial;
    }

    public void setIdMaterial(String idMaterial) {
        this.idMaterial = idMaterial;
    }

    public String getRole() {
        return role;
    }

    public void setRole(String role) {
        this.role = role;
    }

    public String getColor() {
        return color;
    }

    public void setColor(String color) {
        this.color = color;
    }

    public String getColorDesc() {
        return colorDesc;
    }

    public void setColorDesc(String colorDesc) {
        this.colorDesc = colorDesc;
    }

    public String getResult() {
        return result;
    }

    public void setResult(String result) {
        this.result = result;
    }

    public String getFlagSave() {
        return flagSave;
    }

    public void setFlagSave(String flagSave) {
        this.flagSave = flagSave;
    }

    public String getVisual() {
        return visual;
    }

    public void setVisual(String visual) {
        this.visual = visual;
    }

    public String getVisualDesc() {
        return visualDesc;
    }

    public void setVisualDesc(String visualDesc) {
        this.visualDesc = visualDesc;
    }

    public String getCreateBy() {
        return createBy;
    }

    public void setCreateBy(String createBy) {
        this.createBy = createBy;
    }

    public String getUpdateBy() {
        return updateBy;
    }

    public void setUpdateBy(String updateBy) {
        this.updateBy = updateBy;
    }

    public String getFlagResult() {
        return flagResult;
    }

    public void setFlagResult(String flagResult) {
        this.flagResult = flagResult;
    }

    public String getIdAnalyze() {
        return idAnalyze;
    }

    public void setIdAnalyze(String idAnalyze) {
        this.idAnalyze = idAnalyze;
    }
}
