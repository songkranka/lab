...
//	       	if(item.DEFAULT_FLG_M =class= 'Y'&& item.DEFAULT_FLG_T == 'Y' &&item.DEFAULT_FLG_U == 'Y' ){		
				   ...
				   det += '<td class="text-center" >' + item.SPECBASICTEXT + '</td>';
				   ...
				   jsondata['itemid']=item.ITEM_ID;
	            	...
	            	if("0005"==item.ITEM_ID || "0006"==item.ITEM_ID || "0007"==item.ITEM_ID 
	            		...
	            		if("100000002"==item.PRODUCT_ID || "100000003"==item.PRODUCT_ID ||
	            			...
	            			//console.log("no add");
	            			det += '<td class="text-left" ><select class="settools" foo="'+item.ITEM_ID+'"  id="setTool'+index+'">';
	            		}
	            		
	            	}else{
	            		det += '<td class="text-left" ><select class="settools" id="setTool'+index+'">';
	            	}
...
//	       	}         
       ...
       table = $('#assiDetialTWork').DataTable({
		   ...
		   for(var k =0;k<colorObj.length;k++){
			 $('#'+colorObj[k].id).val(colorObj[k].value);
		   }
