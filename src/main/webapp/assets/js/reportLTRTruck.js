		...
		$.each($("#nonSampleType").val(),function(index,items){
	    	...
	    	if("LTR"==$('#nonTypeReport').val()){
				if("00006"==items.toString()){
					 button +='<button type="button" class="btn btn-primary col-xs-offset-5 col-xs-2"  onclick="exportExcelHeaderTools(idTable'+items+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Tools</button>'
					 button +='<button type="button" class="btn btn-primary col-xs-2 col-xs-2"  onclick="exportExcelHeader(idTable'+items+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Result</button>'
					button += '<button type="button" class="btn btn-primary col-xs-3" style="width: 181px;" foo='+labelTemplate.trim()+' onclick="exportReportLTR(idTable'+items+',this)"><i class="fa fa-print"></i>  Print LTR </button>'
				}else{
					button +='<button type="button" class="btn btn-primary col-xs-offset-5 col-xs-2"  onclick="exportExcelHeaderTools(idTable'+items+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Tools</button>'
					button +='<button type="button" class="btn btn-primary col-xs-2 col-xs-2"  onclick="exportExcelHeader(idTable'+items+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Result</button>'
					button += '<button type="button" class="btn btn-primary col-xs-3" style="width: 181px;" foo='+labelTemplate.trim()+' onclick="exportReportLTR(idTable'+items+',this)"><i class="fa fa-print"></i>  Print LTR </button>'
				...
				if("00006"==items.toString()){			
					...
					button +='<button type="button" class="btn btn-primary col-xs-offset-5 col-xs-2"  onclick="exportExcelHeaderTools(idTable'+items+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Tools</button>'
					button +='<button type="button" class="btn btn-primary col-xs-2 col-xs-2"  onclick="exportExcelHeader(idTable'+items+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Result</button>'
					button += '<button type="button" class="btn btn-primary col-xs-3" style="width: 181px;" foo='+labelTemplate.trim()+' onclick="exportReportLTR(idTable'+items+',this)"><i class="fa fa-print"></i>  Print COQ </button>'
				}
	    	}
...
//	    	                        if(o.ASSIGN_STATUS == null || o.ASSIGN_STATUS == '' || o.ASSIGN_STATUS == 'null'){
							...
							{"data" : "plantName","className" : "text-center text-nowrap" ,"value":"plantName", orderable : true},
		...
		$.each($dropdownSampleType,function(index,items){
	    	...
	    	if("LTR"==$('#nonTypeReport').val()){
				if("00006"==items.SAMPLE_TYPE_CODE.toString()){
					...
					button += '<button type="button" class="btn btn-primary col-xs-3" style="width: 181px;" foo='+labelTemplate.trim()+' onclick="exportReportLTR(idTable'+items.SAMPLE_TYPE_CODE+',this)"><i class="fa fa-print"></i>  Print LTR </button>'
				}else{
					button +='<button type="button" class="btn btn-primary col-xs-offset-7 col-xs-2"  onclick="exportExcelHeader(idTable'+items.SAMPLE_TYPE_CODE+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Result</button>'
					button += '<button type="button" class="btn btn-primary col-xs-3" style="width: 181px;" foo='+labelTemplate.trim()+' onclick="exportReportLTR(idTable'+items.SAMPLE_TYPE_CODE+',this)"><i class="fa fa-print"></i>  Print LTR </button>'
				}
				...
				if("00006"==items.SAMPLE_TYPE_CODE.toString()){			
					...
					button +='<button type="button" class="btn btn-primary col-xs-offset-7 col-xs-2"  onclick="exportExcelHeader(idTable'+items.SAMPLE_TYPE_CODE+')" style="margin-right: 10px;"><i class="fa fa-print"></i>  Print Result</button>'
					button += '<button type="button" class="btn btn-primary col-xs-3" style="width: 181px;" foo='+labelTemplate.trim()+' onclick="exportReportLTR(idTable'+items.SAMPLE_TYPE_CODE+',this)"><i class="fa fa-print"></i>  Print COQ </button>'
				}
	    	}
...
function ajaxGetDataReportLTR(){
	if(!$('#apass').is(":checked")&&!$('#npass').is(":checked")&&!$('#upass').is(":checked")){
		showMsgError('กรุณาเลือก Result อย่างใดอย่างหนึ่ง');
		return null;
	}
	var product = "";
	var sampleType = "";
	...
	if($("#nonTypeReport").val()=="LTR"){
		...
		appcheck=appcheck.substring(0,appcheck.length-1);
	var obj  ={
			startDate : $('#sdate').val(),
			endDate : $('#edate').val(),
			productType :product,
...
function exportExcelHeaderTools(data){
	...
	var url= basePath+'reportExcelHeaderTools?labCode='+arr+'&reportType='+$('#nonTypeReport').val();
	 window.location=url;
...
function exportReportLTR(data,productType){
	var arr ="";
	var product = data.id.split("idTable")[1];//idTable00004//$(productType).attr('foo')
	var check = $('#'+data.id+' tbody tr');
	$.each(check,function(index,items){
		if($(this).find('td:eq(1) input')[0].checked){
			if(arr==""){
